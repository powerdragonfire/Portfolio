...
export function ProjectCard({ project, titleAs }: { project: ProjectItemType, titleAs?: keyof React.JSX.IntrinsicElements }) {
  const baseHref = project.link.href.startsWith('http') || project.link.href.startsWith('#')
    ? project.link.href
    : `https://${project.link.href}`
  const utmLink = utm_source ? `${baseHref}${baseHref.includes('?') ? '&' : '?'}utm_source=${utm_source}` : baseHref
